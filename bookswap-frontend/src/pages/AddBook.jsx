...
export default function AddBook() {
  ...
  const { addBook, categories = [], fetchCategories, isLoading } = useBookStore();
  ...
  const [formData, setFormData] = useState({
    ...
    language: 'English',
    imageUrl: '',
    availabilityType: 'SWAP', // Default to SWAP
    rentalPricePerDay: '',
  });
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className={`flex items-center justify-center p-3 border rounded-lg cursor-pointer transition-all ${
                  formData.availabilityType === 'SWAP' 
                  ...
                  <input
                    ...
                    value="SWAP"
                    checked={formData.availabilityType === 'SWAP'}
                    onChange={handleChange}
                    className="sr-only"
                  />
                ...
                <label className={`flex items-center justify-center p-3 border rounded-lg cursor-pointer transition-all ${
                  formData.availabilityType === 'BORROW' 
                    ? 'bg-blue-600 text-white border-blue-600' 
                    : 'bg-gray-50 text-gray-700 border-gray-300 hover:bg-gray-100'
                }`}>
                  <input
                    ...
                    name="availabilityType"
                    value="BORROW"
                    checked={formData.availabilityType === 'BORROW'}
                    onChange={handleChange}
                    className="sr-only"
                  />
                ...
                <label className={`flex items-center justify-center p-3 border rounded-lg cursor-pointer transition-all ${
                  ...
                  <input
                    ...
                    name="availabilityType"
                    value="RENT"
                    checked={formData.availabilityType === 'RENT'}
                    onChange={handleChange}
                    className="sr-only"
                  />
                ...
                <label className={`flex items-center justify-center p-3 border rounded-lg cursor-pointer transition-all ${
                  formData.availabilityType === 'DONATE' 
                    ? 'bg-blue-600 text-white border-blue-600' 
                    : 'bg-gray-50 text-gray-700 border-gray-300 hover:bg-gray-100'
                }`}>
                  <input
                    ...
                    name="availabilityType"
                    value="DONATE"
                    checked={formData.availabilityType === 'DONATE'}
                    onChange={handleChange}
                    className="sr-only"
                  />
            ...
            {formData.availabilityType === 'RENT' && (
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Set the daily rental price for this book
                </p>
              </div>
            )}
